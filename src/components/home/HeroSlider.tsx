  ...
  const content = (
    <div className="relative w-full h-[70vh] bg-muted flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 text-center">
        {slide.caption && (
          <p className="text-lg text-foreground bg-background/60 px-4 py-2 inline-block">
            {slide.caption}
          </p>
        )}
      </div>
      ...
      <div className="absolute bottom-4 z-10 flex gap-2">
        {slides.map((_, i) => (
          <button
            ...
            className={`w-2 h-2 rounded-full transition ${i === current ? "bg-foreground" : "bg-foreground/30"}`}
          />
        ))}
      </div>
    </div>
